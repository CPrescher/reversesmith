...
use super::{PaceModel, PaceNeighbor};

const POSITION_TOLERANCE_ANGSTROM: f64 = 1.0e-10;

/// Stateful native PACE evaluator with an accepted-state atomic-energy cache.
pub struct PaceNativeModel {
    model: PaceModel,
    weight: f64,
    maximum_cutoff: f64,
    box_lengths: [f64; 3],
...
impl PaceNativeModel {
    pub fn from_config(
        ...
        let model_file = config
            .model
            .as_deref()
            .ok_or("[ml_potential] pace_native requires model")?;
        let model_path = resolve_path(base_directory, model_file);
        let model = PaceModel::load(&model_path, &configuration.species)?;
        Ok(Self::new(
            model,
            configuration,
    ...
    pub fn new(
        ...
        if !weight.is_finite() || weight < 0.0 {
            return Err("native PACE weight must be finite and non-negative".to_string());
        }
        if configuration.atoms.is_empty() {
            return Err("native PACE requires at least one atom".to_string());
        }
        if configuration
            .box_lengths
            .iter()
        ...
        {
            return Err("native PACE box lengths must be finite and positive".to_string());
        }
        if configuration.atoms.iter().any(|atom| {
            ...
            return Err("native PACE atom positions must be finite".to_string());
        }
        ...
        let maximum_cutoff = model.maximum_cutoff();
        for (axis, length) in configuration.box_lengths.iter().enumerate() {
            if 2.0 * maximum_cutoff >= *length {
                return Err(format!(
                    "native PACE minimum-image evaluation requires box length {axis} ({length}) to exceed twice the maximum cutoff ({})",
                    2.0 * maximum_cutoff
                ));
            }
        }
        ...
        let accepted_positions = configuration
            ...
            .collect::<Vec<_>>();
        let cell_list = PaceCellList::new(
            &accepted_positions,
    ...
    fn evaluate_atom(
        ...
        let central_type = self.type_indices[atom_index];
        ...
        for neighbor_index in self.cell_list.candidate_atoms(central_position) {
            ...
            let cutoff = self
                .model
                .cutoff_for_types(central_type, self.type_indices[neighbor_index])?;
            if squared_norm(displacement) < cutoff * cutoff {
                neighbors.push(PaceNeighbor {
                    displacement,
        ...
        if let Some((moved_index, new_position)) = trial {
            if moved_index != atom_index {
                ...
                let cutoff = self
                    .model
                    .cutoff_for_types(central_type, self.type_indices[moved_index])?;
                if squared_norm(displacement) < cutoff * cutoff {
                    neighbors.push(PaceNeighbor {
                        displacement,
        ...
        self.model.atomic_energy(central_type, &neighbors)
    }
    ...
    fn affected_atoms(
        ...
        for (position, candidates) in [
            ...
            for candidate in candidates {
                ...
                let cutoff = self
                    .model
                    .cutoff_for_types(self.type_indices[candidate], moved_type)?;
                let displacement = minimum_image_displacement(
    ...
    fn require_no_pending_trial(&self) {
        assert!(
            ...
            "native PACE received a new trial before the previous trial was accepted or rejected"
        );
    }
    ...
    fn debug_assert_cache_consistent(&self) {
        ...
        debug_assert!(
            ...
            "native PACE total energy cache became inconsistent"
        );
    }
}

impl EnergyModel for PaceNativeModel {
    fn label(&self) -> &str {
        "PACE/native"
    }
    ...
    fn total_energy(&mut self, config: &Configuration, _cell_list: &CellList) -> f64 {
        ...
        assert_eq!(
            ...
            "native PACE configuration atom count changed after initialization"
        );
        self.total_energy
    }
    ...
    fn energy_delta_atom(
        ...
        assert!(
            ...
            "native PACE trial positions must be finite"
        );
        ...
        assert!(
            ...
            "native PACE old trial position does not match its accepted state"
        );

        let affected = self
            .affected_atoms(atom_idx, old_position, new_position)
            .unwrap_or_else(|error| panic!("native PACE failed to find affected atoms: {error}"));
            ...
            .map(|index| {
                let energy = self
                    ...
                    .unwrap_or_else(|error| {
                        panic!("native PACE failed to evaluate trial environment: {error}")
                    });
                (index, energy)
            })
            .collect::<Vec<_>>();
    ...
    fn accept_move(&mut self, atom_idx: usize, new_pos: &[f64; 3]) {
        let pending = self
            .pending_trial
            .take()
            .expect("native PACE accept_move called without a pending trial");
        ...
        assert!(
            ...
            "native PACE accepted position differs from its pending trial"
        );
    ...
    fn reject_move(&mut self, atom_idx: usize, old_pos: &[f64; 3]) {
        let pending = self
            .pending_trial
            .take()
            .expect("native PACE reject_move called without a pending trial");
        assert_eq!(pending.atom_index, atom_idx);
        let old_position = wrap_position(*old_pos, self.box_lengths);
        assert!(
            squared_norm(minimum_image_displacement(
            ...
            "native PACE rejected position differs from its accepted state"
        );
    }
}
...
impl PaceCellList {
    ...
    fn move_atom(&mut self, atom_index: usize, new_position: [f64; 3]) {
        ...
        let offset = self.buckets[old_cell]
            ...
            .expect("native PACE cell list lost an atom");
...
mod tests {
    ...
    fn two_cell_periodic_candidate_search_does_not_duplicate_atoms() {
        ...
        let list = PaceCellList::new(&positions, [10.0; 3], 4.9);
