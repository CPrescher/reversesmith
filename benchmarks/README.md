...
| `epsr-silica` | Published EPSR silica-glass example | Demonstrate a chemically plausible network-glass model | First reproduce native EPSR totals and structural distributions, then compare MLIP-HRMC at matched fit quality | Forward/reference-potential parity passed; dense matched-fit recovery favors rsmith for both hidden targets; control/start sensitivity is strong in 11/12 case-arms, with GAP at 1.5x reference strength tied; ambient campaign closed |
| `hp-sio2` | Sequential Erhard-2024 ACE compression | Recover non-affine pressure changes from the preceding-pressure glass | Compare pure RMC, ACE-HRMC, stock EPSR26, and native RMCProfile against hidden 10 GPa increments through 70 GPa | PACE beats both native programs on hidden RDF, common scattering, and Si coordination in 7/7 steps; a matched 20-to-30 GPa domain test shows local `g(r)` improves held-out partial structure while joint `S(Q)+g(r)` is the Pareto compromise; single-seed trends only |
