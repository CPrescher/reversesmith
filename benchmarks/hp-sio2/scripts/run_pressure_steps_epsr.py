#!/usr/bin/env python3
"""Run stock EPSR26 for the incremental pressure steps."""
...
summary = (
    ...
    else {"status": "hp_sio2_pressure_step_native_epsr_runs", "steps": {}}
)
feedback = float(protocol["design"]["native_epsr_feedback"])
...
for step in protocol["steps"]:
    ...
    if args.force or not all(path.is_file() for path in target_files):
        ...
        replay = step_root / "native-forward/replay"
        prepare_run(
            source,
            replay,
            step_root / "inputs/hidden-target.data",
    ...
    prepare_run(
        ...
        int(protocol["design"]["native_epsr_moves_per_refinement"]),
        int(protocol["design"]["native_epsr_refinements"]),
        seed,
        native_targets,
        feedback,
    )
