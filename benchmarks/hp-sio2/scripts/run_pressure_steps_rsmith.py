#!/usr/bin/env python3
"""Run rsmith RMC and PACE arms for the incremental pressure steps."""
...
if not binary.is_file() or not (root / "input-manifest.json").is_file():
    raise SystemExit("missing release binary or prepared pressure steps")
summary_path = root / "rsmith-run-summary.json"
summary = (
    json.loads(summary_path.read_text())
    if summary_path.is_file()
    else {"status": "hp_sio2_pressure_step_rsmith_runs", "runs": {}}
)
environment = {
    **os.environ,
    "RAYON_NUM_THREADS": "1",
