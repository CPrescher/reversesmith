#!/usr/bin/env python3
"""Prepare synthetic 0-to-5 and 5-to-10 GPa recovery fixtures."""
...
source_root = case_root / "reference/local/pressure-steps"
...
if not (source_root / "IMPORT.json").is_file() or not model.is_file():
    raise SystemExit("import pressure sources and the public ACE model first")
...
def write_config(path, structure, neutron, xray, method, seed, moves):
...
backend = "pace_native"
model = "{model}"
weight = {protocol["design"]["pace_weight"]}
'''
...
manifest = {
    ...
    "status": "hp_sio2_pressure_steps_prepared",
    "steps": {},
}
for step in protocol["steps"]:
    name = step["name"]
    step_root = root / name
    inputs = step_root / "inputs"
    inputs.mkdir(parents=True)
    start_positions, start_types, start_box, start_origin = read_lammps(
        source_root / f"{int(step['start_gpa']):02d}gpa.data"
    )
    target_positions, target_types, target_box, target_origin = read_lammps(
        source_root / f"{int(step['target_gpa']):02d}gpa.data"
    )
    ...
    for method in ("rsmith-rmc", "rsmith-pace-w30"):
        ...
        write_config(
            ...
            xray_path,
            method,
            int(protocol["design"]["seed"]),
            int(protocol["design"]["moves"]),
        )
    files = [hidden_path, start_path, neutron_path, xray_path, inputs / "hidden-partial-sq.dat"]
    manifest["steps"][name] = {
