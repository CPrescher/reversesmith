...
source_dir="$epsr_root/workshop2022/EPSR_WorkedExamples_2022/LiquidGa50C"
binary="$epsr_root/bin/epsr"
destination="$case_dir/reference/local"
upstream="$destination/upstream"

if [[ ! -d "$source_dir" ]]; then
    echo "LiquidGa50C worked example not found at: $source_dir" >&2
    echo "Set EPSR26_ROOT to the EPSR installation directory." >&2
    exit 1
fi
if [[ ! -x "$binary" ]]; then
