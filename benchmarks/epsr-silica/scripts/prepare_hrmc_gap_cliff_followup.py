#!/usr/bin/env python3
"""Add the frozen GAP acceptance-cliff follow-up to an existing weight sweep."""
...
protocol = tomllib.loads(
    (case_root / "expected/hrmc-gap-cliff-followup.toml").read_text()
)
root = case_root / "results/hrmc-weight-sweep"
if not root.is_dir():
    raise SystemExit("prepare and run the parent HRMC weight sweep first")

for case in sorted(root.glob("target-*_*")):
    source = case / "gap-w0p3/run.toml"
    if not source.is_file():
        raise SystemExit(f"missing parent configuration {source}")
    for weight in protocol["weights"]["gap"]:
        run = case / f"gap-w{weight_label(weight)}"
